...
fetchDataFromServer(`https://api.themoviedb.org/3/discover/movie?api_key=${api_key}&sort_by=popularity.desc&include_adult=false&page=${currentPage}&${urlParam}`, function({results: movieList,total_pages})
{
    ...
    /**
     * load more button funcitonality
     */
    document.querySelector("[load-more]").addEventListener("click",function()
    {
        if(currentPage>=totalPages)
        {
            this.style.display="none";//this for loading btn
            return;
        }
        currentPage++;
        this.classList.add("loading")// this for loading btn
        ...
        {
            this.classList.remove("loading")//this for loading btn
